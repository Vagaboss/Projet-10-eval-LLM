"""
MistralChat.py — Agent RAG + SQL (fusion intelligente & mixte) pour analyse NBA
"""
...
logfire.configure()
logfire.info("🚀 Démarrage de l’application MistralChat (RAG + SQL mixte)")
...
EXEMPLES :
Question : Quels sont les meilleurs scoreurs ?
SQL :
...
Question : Quels joueurs ont les meilleurs pourcentages au lancer franc ?
SQL :
SELECT p.name, s.ft_pct
FROM players p
JOIN stats s ON p.player_id = s.player_id
ORDER BY s.ft_pct DESC
LIMIT 5;

Question : Quels sont les meilleurs tireurs à 3 points ?
SQL :
...
# --- Détection automatique SQL / Mixte ---
def is_sql_question(prompt: str) -> bool:
    """Détecte si la question nécessite une requête SQL."""
    patterns = [
...
if prompt := st.chat_input("Pose ta question sur la NBA..."):
    ...
    if is_sql_question(prompt):
        ...
        # --- Si la question implique des sources écrites → activer le RAG ---
        if use_rag:
...
st.markdown("---")
st.caption("⚙️ Powered by Mistral AI, FAISS & PostgreSQL | Agent RAG + SQL mixte et robuste par Logfire")
